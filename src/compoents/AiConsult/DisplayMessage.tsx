...
const DisplayMessage = ({ messages, isLoading = false }: DisplayMessageProps) => {
  ...
  if (!messages) {
    return (
      <div className="flex flex-col items-center justify-center h-full w-full p-8 text-center bg-[#e3ebff] text-gray-800">
        ...
        <h3 className="text-xl font-semibold text-[#3a5199] mb-2">AI法律文档生成器</h3>
        <p className="text-gray-700 max-w-md leading-relaxed">请选择要生成的文档类型，填写相关信息后提交请求</p>
        <div className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-3 text-sm">
          <div className="bg-white p-3 rounded-lg shadow-sm border border-gray-200">
            <p className="font-medium text-[#3a5199]">生成法律合同</p>
            <p className="text-gray-600 text-xs mt-1">例如：房屋租赁合同、劳动合同等</p>
          </div>
          <div className="bg-white p-3 rounded-lg shadow-sm border border-gray-200">
            <p className="font-medium text-[#3a5199]">生成投诉状</p>
            <p className="text-gray-600 text-xs mt-1">例如：消费者权益、合同纠纷等</p>
          </div>
          <div className="bg-white p-3 rounded-lg shadow-sm border border-gray-200">
            <p className="font-medium text-[#3a5199]">生成法律意见</p>
            <p className="text-gray-600 text-xs mt-1">例如：法律风险分析、法律建议等</p>
          </div>
        </div>
      </div>
    )
  ...
  return (
    <div ref={containerRef} className="flex flex-col space-y-4 h-full w-full bg-[#e3ebff] text-gray-800">
